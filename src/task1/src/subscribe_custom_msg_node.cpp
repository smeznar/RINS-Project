...
#include <ros/ros.h>
#include "task1/testMsg.h" //The message type

//This is a callback function. It executes every time a new pose message arrives
void messageReceived(const task1::testMsg::ConstPtr& msg){	//The parameter is the message type

	ROS_INFO("I recieved the string: %s", msg->content.c_str());
	ROS_INFO("I recieved the int: %d", msg->n);
}
